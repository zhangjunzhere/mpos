...
import com.itertk.app.mpos.map.BaiduMapUtilByRacer.GeoCodePoiListener;
import com.itertk.app.mpos.map.BaiduMapUtilByRacer.LocateListener;
import com.itertk.app.mpos.map.BaiduMapUtilByRacer.PoiSearchListener;

public class BaiduMapChoiceAddressActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_baidumap_choiceaddress);
		...
		lvAroundPoi = (ListView) findViewById(R.id.lvAroundPoi);
		...
		llMLMain = (FrameLayout) findViewById(R.id.llMLMain);
		// 地图初始化
        ...
        findViewById(R.id.btn_uploadGps).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mLocationBean == null) {
                    Toast.makeText(mContext, "定位中，请稍候", Toast.LENGTH_SHORT).show();
                } else {
                    LinkeaRequest.GpsMsgUpload(BaiduMapChoiceAddressActivity.this, String.valueOf(mLocationBean.getLatitude()), String.valueOf(mLocationBean.getLongitude()));
                }
            }
        });
	}
	...
	private void iniEvent() {
		etMLCityPoi.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (etMLCityPoi.getText().toString().trim().length() > 0) {
					Log.e("etMLCityPoi", "setOnClickListener");
					getPoiByPoiSearch();
				}
			}
		});
		etMLCityPoi.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence cs, int start, int before,
					int count) {
				Log.e("onTextChanged", "setOnClickListener");
		...
		lvAroundPoi.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View v, int position,
					long arg3) {
				mAroundPoiAdapter.setSelected(position);
				isCanUpdateMap = false;
                double latitude = aroundPoiList.get(position).location.latitude;
                double longitude = aroundPoiList.get(position).location.longitude;
				...
				Toast.makeText(mContext, aroundPoiList.get(position).name,
						Toast.LENGTH_LONG).show();
			}
		});
